...
import '../../models/jenis_aksesoris_model.dart';
import '../../models/aksesoris_model.dart';
import '../../services/rest_api_client_service.dart';

class JenisAksesorisController extends GetxController {
  var list = <JenisAksesorisModel>[].obs;
  var selectedList = <JenisAksesorisModel>[].obs;
    ...
    try {
      ...
      if (apiResponse.status == true) {
        for (var i in apiResponse.data) {
          var jsonRes = JenisAksesorisModel.fromJson(i);
          list.add(jsonRes);
        }
  ...
  DataGridSource dSource(BuildContext context, {performSorting}) {
    return BaseDataSource<AksesorisModel>(
      context,
      list
          .map<DataGridRow>(
            (e) => DataGridRow(
              cells: [
                ...
                DataGridCell<int>(
                  ...
                  value: e.idJenisAksesoris,
                ),
                DataGridCell<String?>(
                  columnName: 'jenisAksesoris',
                  value: e.jenisAksesoris,
                ),
              ],
            ),
          )
